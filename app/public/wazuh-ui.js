...
function wazuhDockerText(mapping) {
  const envs=Array.isArray(mapping?.dockerEnvironments)?mapping.dockerEnvironments:[];
  return envs.map(x=>[x.portainerName||'Portainer',x.environmentName||(`Environment ${x.endpointId||''}`)].filter(Boolean).join(' / ')).join(' · ');
}
...
function wazuhVulnerabilityRow(v={}) {
  ...
  return `<article class="wazuh-vuln-row" data-wazuh-row data-severity="${esc(v.severity)}" data-search="${esc(search)}">
    ...
    <div class="wazuh-vuln-foot"><div>${mapping?`<span class="wazuh-correlation">↳ ${esc(wazuhMappingText(mapping))}</span>${wazuhDockerText(mapping)?`<span class="wazuh-docker-correlation">◆ ${esc(wazuhDockerText(mapping))}</span>`:''}`:badge('Non associé à Proxmox','neutral')}</div><div class="row-actions">${button(mapping?'Modifier association':'Associer VM/LXC',`wazuh-map:${encodeURIComponent(v.agentId||v.agentName||'')}`,'tiny')}${state.wazuhOverview?.integration?.dashboardUrl?`<a class="btn tiny secondary" target="_blank" rel="noopener noreferrer" href="${esc(state.wazuhOverview.integration.dashboardUrl)}">Ouvrir Wazuh</a>`:''}</div></div>
  </article>`;
}
function wazuhAlertRow(a={}) {
  const mapping=a.mapping||wazuhMachineMapping(a.agentId);
  return `<article class="wazuh-alert-row"><div class="wazuh-alert-level ${Number(a.level||0)>=15?'critical':''}">${Number(a.level||0)}</div><div class="wazuh-alert-copy"><div><strong>${esc(a.description||'Alerte Wazuh')}</strong>${badge(`Règle ${a.ruleId||'—'}`,'neutral')}</div><p>${esc(a.agentName||a.agentId||'Endpoint')} · ${esc(fmtDate(a.timestamp))}</p>${a.mitreTactics?.length?`<small>MITRE · ${esc(a.mitreTactics.join(', '))}${a.mitreTechniques?.length?` · ${esc(a.mitreTechniques.join(', '))}`:''}</small>`:''}${mapping?`<small class="wazuh-correlation">↳ ${esc(wazuhMappingText(mapping))}</small>${wazuhDockerText(mapping)?`<small class="wazuh-docker-correlation">◆ ${esc(wazuhDockerText(mapping))}</small>`:''}`:''}</div></article>`;
}
function wazuhPage() {
...
function wazuhMachineSecurityMarkup(machine={}) {
  ...
  return `<section class="panel machine-detail-section wazuh-machine-security"><div class="machine-detail-section-head"><div><h3>Wazuh Security</h3><span>Agent ${esc(ep.agentName||ep.agentId)}</span></div>${badge(wazuhPriorityLabel(ep.priority),wazuhPriorityTone(ep.priority))}</div><div class="wazuh-machine-summary"><div><span>CVE critiques</span><strong>${ep.critical||0}</strong></div><div><span>CVE élevées</span><strong>${ep.high||0}</strong></div><div><span>Alertes importantes</span><strong>${ep.alerts||0}</strong></div><div><span>Agent</span><strong>${ep.disconnected?'Déconnecté':'Actif'}</strong></div></div>${ep.mapping&&wazuhDockerText(ep.mapping)?`<div class="wazuh-machine-docker"><strong>Docker corrélé</strong><span>◆ ${esc(wazuhDockerText(ep.mapping))}</span><small>Corrélation par environnement Portainer associé à cette VM/LXC. Aucun conteneur n’est attribué sans preuve fiable.</small></div>`:''}${vulns.length?`<div class="wazuh-machine-packages">${[...new Set(vulns.map(v=>v.packageName).filter(Boolean))].slice(0,8).map(p=>`<code>${esc(p)}</code>`).join('')}</div>`:''}<div class="actions">${button('Ouvrir Wazuh Security','go-wazuh','secondary')}</div></section>`;
}
