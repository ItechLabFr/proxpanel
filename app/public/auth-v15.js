function renderAuth(setup){
  ...
  const setBusy=(busy,label='')=>{submitting=busy;if(submitBtn){submitBtn.disabled=busy;const span=qs('span',submitBtn);if(span)span.textContent=busy?(label||at('Connexion…','Signing in…')):(setup?at('Créer le compte','Create account'):at('Se connecter','Sign in'))}qsa('.mfa-verify',form).forEach(button=>button.disabled=busy)};
  ...
  const renderRecovery=()=>{
    ...
    slot.innerHTML=`<div class="login-2fa login-2fa-v15">${mfaHeader(at('Code de récupération','Recovery code'),at('Utilise l’un des codes enregistrés lors de l’activation de la 2FA.','Use one of the codes saved when two-factor authentication was enabled.'))}<label class="auth-field"><span>${at('Code de récupération','Recovery code')}</span><input id="authRecoveryCode" autocomplete="one-time-code" required autocapitalize="characters" spellcheck="false" placeholder="XXXX-XXXX-XXXX-XXXX"></label><button type="submit" class="btn primary mfa-verify">${at('Continuer','Continue')}</button><button type="button" class="mfa-back" id="backTotp">← ${at('Utiliser Authenticator','Use Authenticator')}</button></div>`;
